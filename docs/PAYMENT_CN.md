...
> **部署前置要求**：必须配置并永久保存固定的 `TOTP_ENCRYPTION_KEY`（64 位十六进制）。支付服务商密钥使用该密钥进行 AES-256-GCM 加密；未配置固定密钥时，后台会拒绝新增或修改支付服务商，避免凭证以明文写入数据库。Docker 部署脚本会自动生成并写入 `.env`，手工部署可使用 `openssl rand -hex 32` 生成。
...
`Secret Key` 与 `Publishable Key` 必须属于同一模式：测试环境使用 `sk_test_...` + `pk_test_...`，生产环境使用 `sk_live_...` + `pk_live_...`。系统会在保存时检查必填项和模式是否匹配。多 Stripe 实例场景下，每笔订单会使用负载均衡实际选中实例的 Publishable Key 和 Webhook Secret，不会混用其他实例的配置。
...
- Stripe Webhook 必须发送原始请求体，并订阅 `payment_intent.succeeded`、`payment_intent.payment_failed`；系统以 PaymentIntent 的 `metadata.orderId` 定位实际收款实例
...
- 后台任务每 60 秒执行一次超时检查
- 前端只有在订单进入 `COMPLETED` 后才显示“充值成功”；`PAID` 和 `RECHARGING` 均显示“处理中”
- 站内支付二维码弹窗禁止点击遮罩空白区域或按 `Esc` 关闭，只能通过明确的取消、关闭按钮操作
- Stripe 微信支付二维码由站内页面直接展示，不使用可被遮罩点击关闭的 Stripe 内置二维码弹层；用户主动关闭浏览器支付窗口也不会改变订单状态或被当作支付成功
