...
const METHOD_ICONS: Record<string, string> = {
  ...
  wxpay: wxpayIcon,
  stripe: stripeIcon,
  stripe_card: paymentIcon,
  stripe_alipay: alipayIcon,
  stripe_wxpay: wxpayIcon,
  stripe_link: stripeIcon,
...
function methodLabel(method: PaymentMethodOption): string {
  if (method.type.startsWith('stripe_')) {
...
function methodSelectedClass(type: string): string {
  if (isBuiltInAlipayMethod(type) || type === 'stripe_alipay') return 'border-[#02A9F1] bg-blue-50 text-gray-900 shadow-sm dark:bg-blue-950 dark:text-gray-100'
  if (isBuiltInWxpayMethod(type) || type === 'stripe_wxpay') return 'border-[#09BB07] bg-green-50 text-gray-900 shadow-sm dark:bg-green-950 dark:text-gray-100'
  if (type === 'stripe' || type === 'stripe_card' || type === 'stripe_link') return 'border-[#676BE5] bg-indigo-50 text-gray-900 shadow-sm dark:bg-indigo-950 dark:text-gray-100'
