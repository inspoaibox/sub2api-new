...
import { getPaymentPopupFeatures, getQrPaymentPopupFeatures, isBuiltInAlipayMethod, isBuiltInWxpayMethod } from '@/components/payment/providerConfig'
...
function reopenPopup() {
  if (props.payUrl) {
    const popupFeatures = props.paymentType === 'stripe_wxpay'
      ? getQrPaymentPopupFeatures()
      : getPaymentPopupFeatures()
    const win = window.open(props.payUrl, 'paymentPopup', popupFeatures)
    if (!win || win.closed) {
