...
export const useAppStore = defineStore('app', () => {
  ...
  const canSelfUpdate = ref<boolean>(false)
  const updateRepository = ref<string>('inspoaibox/sub2api-new')
  ...
  async function fetchVersion(force = false): Promise<VersionInfo | null> {
    ...
    if (versionLoaded.value && !force) {
      return {
        ...
        can_self_update: canSelfUpdate.value,
    ...
    try {
      ...
      canSelfUpdate.value = data.can_self_update ?? buildType.value === 'release'
      updateRepository.value = data.update_repository || 'inspoaibox/sub2api-new'
